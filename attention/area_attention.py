...
from .tensor_utils import shape_list, cast_like
from .attention_utils import combine_heads
...
def basic_pool(features, max_area_width, max_area_height=1, height=1,
  ...
  """
  feature_shape = shape_list(features)
...
def _compute_sum_image(features, max_area_width, max_area_height=1, height=1,
  ...
  Returns:
    ...
    area_widths: A Tensor of shape [batch_size, num_areas, 1]
  """
  # TODO: convert to x.shape.as_list()?
  feature_shape = shape_list(features)
  batch_size = feature_shape[0]
  length = feature_shape[-2]
  depth = feature_shape[-1]
...
def compute_area_features(features, max_area_width, max_area_height=1, height=1,
  ...
  area_squared_sum, _, _ = _compute_sum_image(
      ...
      max_area_height=max_area_height, height=height)
  sizes = tf.math.multiply(area_heights, area_widths)
  float_area_sizes = tf.cast(sizes, dtype = tf.float32)
  area_mean = tf.math.divide(area_sum, float_area_sizes)
  s2_n = tf.math.divide(area_squared_sum, float_area_sizes)
  area_variance = tf.math.subtract(s2_n, tf.pow(area_mean, 2))
...
def compute_area_key(features, max_area_width, max_area_height=1, height=1,
  ...
  depth = shape_list(area_mean)[-1]
  height_embed = tf.nn.embedding_lookup(
      params=tf.Variable(tf.random.normal([max_area_height, depth // 2]),
                         name = "area_height_emb"),
      ids=area_heights[:, :, 0] - 1)
  width_embed = tf.nn.embedding_lookup(
      params=tf.Variable(tf.random.normal([max_area_width, depth // 2]), 
                         name = "area_width_emb"),
      ids=area_widths[:, :, 0] - 1)
  ...
  feature_hidden = tf.keras.layers.Dense(inputs=feature_concat,
                                         units=depth,
                                         activation=tf.nn.relu)
  area_key = tf.keras.layers.Dense(feature_hidden, units=depth)
  return area_key


def py_dot_product_area_attention(q,
                                  k,
                                  v,
                                  bias=None,
                                  dropout_rate=0.0,
  ...
  # TODO: change to x.shape.as_list()?
  mem_shape  = shape_list(k)
  ...
  if bias is not None:
    ...
    _, _, padding_sum, _, _ = compute_area_features(
        ...
        max_area_height=max_area_height, height=memory_height)
    bias = tf.where( # TODO: investigate the tf.where behavior changes
        tf.cast(tf.cast(padding_sum, tf.int32), tf.bool),
  ...
  if top_k_areas > 0:
    ...
    top_k = tf.minimum(shape_list(weights)[-1], top_k_areas)
    ...
    weights = tf.math.divide(weights, tf.reduce_sum(weights, -1, keepdims=True))
